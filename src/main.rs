use aoc25::*;

fn main() {
    println!("day1p1: {}", day1p1(include_str!("../input/1")));
    println!("day1p2: {}", day1p2(include_str!("../input/1")));
    println!("day2p1: {}", day2p1(include_str!("../input/2")));
    println!("day2p2: {}", day2p2(include_str!("../input/2")));
    println!("day3p1: {}", day3p1(include_str!("../input/3")));
    println!("day3p2: {}", day3p2(include_str!("../input/3")));
    println!("day4p1: {}", day4p1(include_str!("../input/4")));
    println!("day4p2: {}", day4p2(include_str!("../input/4")));
    println!("day5p1: {}", day5p1(include_str!("../input/5")));
    println!("day5p2: {}", day5p2(include_str!("../input/5")));
    println!("day6p1: {}", day6p1(include_str!("../input/6")));
    println!("day6p2: {}", day6p2(include_str!("../input/6")));
    println!("day7p1: {}", day7p1(include_str!("../input/7")));
    println!("day7p2: {}", day7p2(include_str!("../input/7")));
}
